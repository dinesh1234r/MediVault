...
import {
  Box, Button, Input, Heading, VStack, HStack, Radio, Image, RadioGroup, Stack, useToast, FormControl, FormLabel, Text, Progress, IconButton
...
const AddDoctors = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Validation
      if (!values.Doctor_name || !values.Email_Address || !values.Current_Address || !photo || !values.Qualifications || !values.Specialization || !values.Medical_License_Number || !values.Medical_Council_Registration_Number || !values.Years_of_experience || !values.Contract_type || !gender) {
        toast({
      ...
      const response = await axios.post('https://medivault.onrender.com/admin/postbyadmin', {
      ...
      if (response.data.msg === "Details are saved successfully") {
        toast({ title: response.data.msg, status: "success", duration: 1500, isClosable: true, position: 'top' });
        setValues({ Doctor_name: "", Email_Address: "", Current_Address: "", DOB: "", Qualifications: "", Specialization: "", Medical_License_Number: "", Medical_Council_Registration_Number: "", Years_of_experience: "", Contract_type: "" });
        setGender("");
        setPhoto("");
        setImage("");
      } else {
  ...
  return (
    <Box bg="gray.100" p={6} ml={{ base: 0, md: '250px' }} transition="margin-left 0.2s ease" w={'100%'}>
      <Box width="90%" mx="auto" p={6} bg="white" borderRadius="lg" boxShadow="md">
        <Heading size="lg" textAlign="center" mb={4}>Add Doctor</Heading>
        ...
        {page === 1 && (
          <VStack spacing={4} align="start">
            <FormControl>
              ...
              <Input name="Doctor_name" value={values.Doctor_name} onChange={handleChange} placeholder="Enter Doctor's Name" />
            </FormControl>
            <FormControl>
        ...
        {page === 2 && (
          <VStack spacing={4} align="start">
            ...
            <FormControl>
              ...
              <Input name="Contract_type" value={values.Contract_type} onChange={handleChange} placeholder="Enter Contract Type" />
            </FormControl>
          </VStack>
        )}
